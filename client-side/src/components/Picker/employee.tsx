...
export const EmployeePicker: FC<IProps & Partial<BasicModelPickerProps>> = (props) => {
    const api = useAPI();
    ...
    return <BasicModelPicker
        loadOption={async (input) => {
            const res = await api.getWithToken(
                `${appConfig.backendUri}/account/manager/select?roleId=2&${filterClassId}${filterExamTestId}searchvalue=${String(input)}&searchby=fullname&orderby=fullname&orderdirection=asc&start=0&count=5`
            );
