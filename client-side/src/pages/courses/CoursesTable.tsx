...
export const CoursesTable: FC<IProps> = (props) => {
    ...
    function renderRow(item: IData) {
        return <Grid key={item.id} item xs={12} sm={6} md={4}>
            <CourseCard name={item.name} price={item.price} location={item.location} dateStart={item.dateStart} dateEnd={item.dateEnd} maxMember={item.maxMember} item={item} onDelete={() => props.onDelete} onEdit={() => props.onEdit} countMember={item.countMember} employeeCNNs={item.employeeCNNs}></CourseCard>
        </Grid>
    }
